...
class Minify
{
   ...
   * @var string
   */
  public $js_tag = '<script type="text/javascript" src="%s"  %s ></script>';
  /**
  ...
  private function _output($files, $type)
  {
    ...
    foreach ($files as $file)
    {
      if($type=='js' && is_array($file)){
        $output[] = $this->html_tags ? sprintf($template, $this->_base_url($file),'async') : $this->_base_url($file);
      }else{
  ...
  private function _simple_output($type, $group)
  {
    ...
    foreach ($files as $file)
    {
      if(is_array($file)) $filename = $directory . '/' . $file[0];
      else $filename = $directory . '/' . $file;
